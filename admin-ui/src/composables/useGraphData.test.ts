...
describe('useGraphData', () => {
  ...
  it('filterTrustlinesByEqAndStatus filters by eq and status', () => {
    ...
    expect(
      filterTrustlinesByEqAndStatus({ trustlines: trustlines as any, equivalent: 'EUR', statusFilter: ['active', 'closed'] }).map(
        (t) => t.to
      )
    ).toEqual(['B', 'C'])

    expect(
      filterTrustlinesByEqAndStatus({ trustlines: trustlines as any, equivalent: 'EUR', statusFilter: ['active'] }).map((t) => t.to)
    ).toEqual(['B'])

    expect(
      filterTrustlinesByEqAndStatus({ trustlines: trustlines as any, equivalent: 'ALL', statusFilter: ['active'] }).map((t) => t.to)
    ).toEqual(['B', 'D'])
  })
  ...
  it('computeIncidentRatioByPid filters by eq and keeps max ratio per pid', () => {
    ...
    const m = computeIncidentRatioByPid({ incidents: incidents as any, equivalent: 'EUR' })
  ...
  it('availableEquivalents merges dataset + trustlines and always includes ALL', () => {
    ...
    g.equivalents.value = [{ code: 'eur', precision: 2 } as any]
    g.trustlines.value = [{ equivalent: ' usd ' } as any, { equivalent: 'EUR' } as any]
