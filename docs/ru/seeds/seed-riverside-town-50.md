...
### Откуда взяты эти циклы

Оба цикла ниже — **не иллюстрация, а выписка из `seeds/communities/riverside-town-50/recipe.json`**. Каждое ребро проверено против `community.json` 2026-09-22: линия существует в нужном направлении, несёт `auto_clearing: true`, сумма укладывается в лимит. Прежние примеры этого раздела называли пары, между которыми в v2-топологии линий нет вовсе; они заменены, а не подправлены.

Направление стрелок ниже — **должник → кредитор** (граф долгов), как сказано выше.

### Cycle 1: тот, который гасится (3 узла, UAH)

```mermaid
graph LR
    M[Fish Market<br/>& Cold Storage] -->|owes 1150.00 UAH| P[Petro Rybka<br/>Fisherman]
    P -->|owes 940.00 UAH| C[Riverside Fishing<br/>Co-operative]
    C -->|owes 1480.00 UAH| M
```

**Как он возникает.** Рынок забрал недельный улов Петра, но платит за него позже — так рынок стал должен Петру. Пётр взял у кооператива снасти в кредит. Кооператив арендует у рынка холодильник.

**Клиринг**: `min(1150.00, 940.00, 1480.00) = 940.00 UAH` гасится по кругу. После: долг Петра кооперативу **0.00**, рынок должен Петру 210.00, кооператив должен рынку 540.00. Суммарный долг в UAH падает на 2820.00 — по 940.00 на каждом из трёх рёбер, — а **нетто-позиция каждого участника не меняется**: в этом весь смысл клиринга.

### Cycle 2: тот, который остаётся (3 узла, UAH)

```mermaid
graph LR
    O[Olena Kuhnia<br/>Catering] -->|owes 880.00 UAH| I[Ivan Kozak<br/>Fisherman]
    I -->|owes 1015.00 UAH| G[Town Grocery]
    G -->|owes 1240.00 UAH| O
```

**Как он возникает.** Олена закупает у Ивана рыбу для кейтеринга. Иван покупает продукты в бакалее. Бакалея заказывает у Олены готовые блюда на витрину.

Этот цикл строится **после** команды клиринга и потому её переживает; клирингуемая сумма — `min(880.00, 1015.00, 1240.00) = 880.00 UAH`. Он нужен, чтобы на экране было видно не только результат клиринга, но и то, что в сети ещё есть что гасить. Ни одно его ребро не входит в Cycle 1.

### Почему здесь нет примера в HOUR

Прежняя версия раздела показывала цикл в часах. В v2-топологии его **не существует**: все 96 активных HOUR-линий Riverside несут `auto_clearing: false`, поэтому клиринг пропустил бы любой цикл в часах, даже если бы тот замыкался. Это свойство топологии, а не упущение примера. Если демонстрация клиринга в часах понадобится, менять придётся описание сообщества — пример сам по себе этого не даст.
