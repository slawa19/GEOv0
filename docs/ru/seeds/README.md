...
(структура: кто есть кто,    │
 кто кому доверяет)          └────────────►  рецепт операций → долги и балансы в БД  (рецепт есть — T1713; исполнителя нет — T1711)
```
...
Второй потребитель описания — компактный рецепт настоящих операций (регистрации, линии, платежи, клиринг), который исполняется через доменные сервисы и **порождает** долги и балансы. Сам рецепт **существует** с 2026-09-22 — `seeds/communities/<id>/recipe.json` плюс валидатор `seeds/communities/recipe_schema.py` (`T1713`). Чего ещё нет — **исполнителя** (`T1711`): ни одна команда рецепта ни разу не выполнялась на базе, и числа в её поле `expect` выведены из описания, а не измерены прогоном. Ссылаться на рецепт как на источник данных в БД пока нельзя.
