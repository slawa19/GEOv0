...
> Примечание: это **концептуальный документ** (исторический контекст).
>
...
- Профиль:
  ...
  - тип участника (`person`, `business`, `hub` и т.п.);
...
⚠️ **УСТАРЕВШЕЕ для текущего репозитория (реализация GEOv0):** админка реализована как отдельный проект `admin-ui/` на **Vue 3 + TypeScript + Vite + Element Plus + Pinia** и работает через backend Admin API.

Вариант «Jinja2 + HTMX/Alpine.js» оставлен как возможный подход для отдельных диагностических интерфейсов, но не как текущий выбор для админки.
