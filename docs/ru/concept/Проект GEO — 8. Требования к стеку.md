## 8. Требования к стеку

> Примечание (важно): этот документ — **концепт/обоснование выбора стека**.
> 
> **Каноническое описание фактического стека текущей реализации** и источник версий/зависимостей находятся в:
> - `docs/ru/03-architecture.md` (раздел «Технологический стек»)
> - `requirements.txt`, `requirements-dev.txt`, `admin-ui/package.json`
>
> Всё, что ниже про альтернативные варианты и рекомендации, следует читать как **исторический контекст**; если есть расхождения с репозиторием — приоритет у канона.
...
**Dарианты архитектуры** с обоснованием, и затем — конкретная рекомендация.
...
Можно сделать (как концепт):

- сервер‑рендеринг HTML на Python (Jinja2);
- чуть‑чуть прогрессивного JavaScript через **HTMX** или **Alpine.js**.

⚠️ **УСТАРЕВШЕЕ для текущего репозитория:** на практике админка в реализации GEOv0 сделана как отдельный SPA-проект `admin-ui/` на **Vue 3 + TypeScript + Vite + Element Plus + Pinia**, который обращается к backend Admin API.

Идея «минимального не-SPA web-интерфейса» остаётся возможной для отдельных диагностических сценариев, но это **не текущий путь реализации админки**.
...
- **Web‑админка (текущая реализация):**
   отдельный проект `admin-ui/`: **Vue 3 + TypeScript + Vite + Element Plus + Pinia**, интеграция через backend Admin API.

- **Минимальный web‑интерфейс узла (альтернатива, неактуально для текущей реализации):**
   сервер‑рендеринг HTML (Jinja2) на стороне backend’а + минимальный JS для интерактивности (HTMX/Alpine.js).
   Такой подход остаётся допустимым как концепт «простого диагностического UI», но **в этом репозитории считается устаревшим выбором для админки**.
