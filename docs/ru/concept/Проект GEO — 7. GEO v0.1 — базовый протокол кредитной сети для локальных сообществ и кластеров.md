...
- Любое изменение amount должно атомарно инкрементировать `version` и проверять `WHERE version = OLD_VERSION`.
...
> Примечание: Все модификации долгов (Debts) должны использовать **Optimistic Locking** (по полю `version`). Если во время подготовки `PREPARE` долг был изменён другой транзакцией (клиринг или параллельный платеж), координатор должен получить от БД ошибку (Stale Data / Conflict), перечитать актуальное состояние и повторить попытку (Retry) или отвергнуть транзакцию (`ABORTED`).
