...
- **Anonymous visitors** ✅ — cookie-based сессии (`geo_sim_sid`, HMAC-SHA256) позволяют анонимным посетителям запускать свои simulator run'ы без регистрации. Per-owner изоляция, admin control plane, CSRF защита. **Полностью реализовано** (64 backend + 217 frontend тестов). См. [спецификацию](backend/anonymous-visitors-cookie-runs-spec.md) и [acceptance criteria](backend/acceptance-criteria.md).
- Спеки UI/визуала:
