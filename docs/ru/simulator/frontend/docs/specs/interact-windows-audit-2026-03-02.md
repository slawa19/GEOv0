...
**Статус: ⚠️ Partial fix (REQ-16)**
Добавлены CSS `min-height` на корневых элементах панелей (`ManualPaymentPanel`, `TrustlineManagementPanel`, `ClearingPanel`, `EdgeDetailPopup`) и `contain: layout style` на `WindowShell` (frameless mode). Это уменьшает 1-frame gap между mount и ResizeObserver callback. Полное устранение (skeleton + offscreen pre-measure) — POST-MVP.
...
#### ARCH-4 (Done): Циклические зависимости watchers

**Статус:** ✅ Done — Добавлено ESLint `no-restricted-syntax` правило для запрета `watchEffect` в WM bridging файлах ([`eslint.config.js`](simulator-ui/v2/eslint.config.js)).
...
- `wmEdgeDetailEffectiveState(win)` в [`SimulatorAppRoot.vue:658-677`](simulator-ui/v2/src/components/SimulatorAppRoot.vue:658) возвращает frozen `fromPid`/`toPid`/`selectedEdgeKey` из `win.data` (WM source of truth) когда `wmEdgeDetail.state === 'keepAlive'`, вместо live `interact.mode.state`.
- `buildOpenArgs()` в [`useWmEdgeDetail.ts:105-119`](simulator-ui/v2/src/composables/useWmEdgeDetail.ts:105) сохраняет `fromPid`, `toPid`, `edgeKey`, `title` в `win.data` при открытии.
- `desiredOpenReq()` в [`useWmEdgeDetail.ts:121-126`](simulator-ui/v2/src/composables/useWmEdgeDetail.ts:121) возвращает `frozenReq` (оригинальный A→B) в keepAlive, игнорируя изменения `autoReq`.

**Тесты:**
- Unit: [`useWmEdgeDetail.test.ts`](simulator-ui/v2/src/composables/useWmEdgeDetail.test.ts) — блок `ARCH-7: keepAlive preserves frozen context (no drift)` с двумя тестами.
- Integration: [`SimulatorAppRoot.interact.test.ts:978`](simulator-ui/v2/src/components/SimulatorAppRoot.interact.test.ts:978) — `ARCH-7: EdgeDetail in keepAlive shows frozen edge context, not live interact state`.
...
|----|-----------|------------|---------|---------|---------|
| UX-1 | UX | P2 | Этап 5 | Видимый ресайз при открытии | ⚠️ Partial fix |
...
| ARCH-4 | Arch | P3 | Этап 3 | Циклические watchers | ✅ Done |
| ARCH-6 | Arch | P2 | Этап 2 | Множество ad-hoc EdgeDetail flags | ✅ Done |
| ARCH-7 | Arch/UX | **P1** | Этап 2 | keepAlive EdgeDetail context drift | ✅ Done |
