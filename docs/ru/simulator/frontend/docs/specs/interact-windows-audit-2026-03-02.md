...
Система управления окнами определена в [`types.ts`](simulator-ui/v2/src/composables/windowManager/types.ts:1):
...
├── useNodeCard()               → legacy positioning (WM bypass)
│   ├── 4-direction placement
│   └── _reclamp nextTick correction
├── useNodeSelectionAndCardOpen()
│   ├── selectNode() → isNodeCardOpen = false
│   └── setNodeCardOpen()
...
Исторически (на момент этого аудита) feature flag [`__USE_WINDOW_MANAGER`](simulator-ui/v2/src/components/SimulatorAppRoot.vue:116) определялся через query string. В текущем коде legacy opt-out удалён, runtime — WM-only.

```
┌─ Legacy path (удалён; исторический контекст) ───────┐
│  <Transition name="panel-slide">                     │
│    v-if !__USE_WINDOW_MANAGER                        │
│    → absolute positioning через useNodeCard()        │
│    → handleEscOverlayStack() для ESC                 │
│    → _reclamp nextTick двухпроходная коррекция       │
└──────────────────────────────────────────────────────┘

┌─ WM path (default; текущий runtime) ────────────────┐
...
| Данные                    | WM mode                          | Legacy mode                      |
|--------------------------|----------------------------------|----------------------------------|
| Позиция окна             | `wm.windowsMap[id].rect`         | CSS через `useNodeCard.nodeCardStyle` |
| Размер окна              | `wm.windowsMap[id].measured`     | Не отслеживается явно             |
| Открыто ли окно          | `windowsMap.has(id)`             | `isNodeCardOpen`, `interactPhase` |
| Активное окно            | `wm.activeId`                    | Нет единого источника             |
| Z-порядок                | `win.z` (focusCounter)           | Нет (фиксированный CSS z-index)  |
| Фаза Interact            | `useInteractFSM.phase` (всегда) | То же                             |
...
**Источник события:** [`useCanvasInteractions.onCanvasDblClick()`](simulator-ui/v2/src/composables/useCanvasInteractions.ts:92)
...
> - никаких `setNodeCardOpen(true/false)` в WM runtime не используется.
...
onCanvasDblClick(ev)
  ...
  │ setNodeCardOpen(true)
  │
  ├─── WM path ──────────────────────────────────────┐
  │                                                   │
  │  useSimulatorApp.__selectNodeFromCanvasStep5()     │
  │    │ selectNode(id)                                │ [useNodeSelectionAndCardOpen.ts:6]
  │    │   selectedNodeId.value = id                   │
  │    │   isNodeCardOpen.value = false  ← ЗАКРЫВАЕТ!  │
  │    │                                               │
  │    │ opts.uiOpenOrUpdateNodeCard({nodeId, anchor}) │ [SimulatorAppRoot.vue:213]
  │    │   wm.open({type:'node-card', anchor, data})   │ [useWindowManager.ts:316]
  │    │     getPolicy() → inspector, reuse, close     │
  │    │     getConstraints() → 320/180/360/260        │ [:177]
  │    │     closeGroupExcept('inspector', id)          │
  │    │     estimateSizeFromConstraints(constraints)   │ [geometry.ts:9]
  │    │     cascadeShiftAvoidOverlaps()               │ [:22]
  │    │     focus(id) → z = ++focusCounter            │
  │    │     reclamp(id) → pad=12, snap8               │
  │    │   wmNodeCardId.value = id                      │
  │    │                                               │
  │  ПАРАЛЛЕЛЬНО: watcher на [isNodeCardOpen, ...]     │ [SimulatorAppRoot.vue:581]
  │    shouldShow = !!nodeId && open && !dragging       │
  │    → wm.open() singleton=reuse → обновляет data    │
  │    → НО isNodeCardOpen уже false (от selectNode)!  │
  │    → shouldShow = false → wm.close()!              │
  │                                                    │
  │  КОНФЛИКТ: uiOpenOrUpdateNodeCard открывает,       │
  │  а watcher (через isNodeCardOpen=false) закрывает  │
  │                                                    │
  │  РЕШЕНИЕ: suppressNextCanvasNodeCardClose guard    │
  │    [useSimulatorApp.ts:581]                         │
  │                                                    │
  └────────────────────────────────────────────────────┘
  │
  ├─── Legacy path ──────────────────────────────────┐
  │  selectNode(id) → isNodeCardOpen = false          │
  │  setNodeCardOpen(true)                            │
  │  → isNodeCardOpen.value = true                    │
  │  → useNodeCard._baseNodeCardStyle computed:       │
  │    worldToScreen → 4-direction placement          │ [useNodeCard.ts:94]
  │    → 3-level fallback                             │ [:206-218]
  │  → nodeCardStyle = base + _reclamp correction     │ [:227]
  │  → nextTick → getBoundingClientRect → _reclamp   │ [:243]
  └──────────────────────────────────────────────────┘
...
Владельцы состояния:
  ...
  - isNodeCardOpen: useNodeSelectionAndCardOpen
  - windowsMap: useWindowManager
  - wmNodeCardId: SimulatorAppRoot

Потенциальные гонки:
  - selectNode сбрасывает isNodeCardOpen → watcher может закрыть WM окно
  - suppressNextCanvasNodeCardClose — хрупкий guard
```
...
User: dblclick на ноду B (при открытой NodeCard для ноды A)
  │
  ▼
onCanvasDblClick(ev)    [useCanvasInteractions.ts:92]
  │ pickNodeAt → hit.id = 'B'
  │ setSelectedNodeId('B')
  │ setNodeCardOpen(true)
  │
  ...
  │  selectNode('B')                                  │
  │    selectedNodeId = 'B'                           │
  │    isNodeCardOpen = false  ← СБРОС!              │
  │                                                   │
  │  uiOpenOrUpdateNodeCard({nodeId:'B', anchor})     │
  │    wm.open({type:'node-card', data:{nodeId:'B'}}) │
  │      singleton=reuse → находим существующее окно  │ [useWindowManager.ts:329]
  │      win.data = {nodeId:'B'}                      │ [:342]
  │      win.anchor = новый anchor для B              │ [:343]
  │      anchorChanged = true                         │ [:334-340]
  │        → repositioning к новому anchor             │ [:359-362]
  │      closeGroupExcept('inspector', id)            │ [:376]
  │      cascadeShiftAvoidOverlaps                    │ [:379-389]
  │      focus + reclamp                              │
  │                                                   │
  │  watcher: isNodeCardOpen=false, nodeId='B'        │
  │    shouldShow = false (open=false)                │
  │    → wm.close(wmNodeCardId) ← ЗАКРЫВАЕТ!         │
  │                                                   │
  │  suppressNextCanvasNodeCardClose guard спасает     │
  │  [useSimulatorApp.ts:581]                          │
  │                                                   │
  │  ИТОГО: окно перемещается к ноде B,              │
  │  data обновляется, anchor меняется.               │
  └────────────────────────────────────────────────────┘

Потенциальные проблемы:
  P1: Если suppress guard не сработал → мерцание (close→reopen)
  P2: Rapid clicks (A→B→C→D) → watchers coalesce, но wm.open()
      вызывается из uiOpenOrUpdateNodeCard синхронно
...
@close emit                                    [SimulatorAppRoot.vue:1391]
  ...
  │
  │ policy.onClose('action')                   [useWindowManager.ts:128]
  │   data.onClose('action')
  │     → setNodeCardOpen(false)               [SimulatorAppRoot.vue:600]
  │     (вызывается дважды — из template И из policy)
  │
  │ windowsMap.delete(id)
  │ wmNodeCardId = null (через watcher)
  │
...
onGlobalKeydown                                [SimulatorAppRoot.vue:712]
  ...
  │         → setNodeCardOpen(false)
  │
  │ isNodeCardOpen=false → watcher fires
  │   → shouldShow=false → wm.close()
  │   → НО окно уже удалено → no-op
...
**Ключевой механизм:** `escBehavior: 'back-then-close'` + [`policy.onEsc()`](simulator-ui/v2/src/composables/windowManager/useWindowManager.ts:78)
...
onGlobalKeydown(ev)                             [SimulatorAppRoot.vue:712]
  │ ev.key === 'Escape'
  │ __USE_WINDOW_MANAGER = true
  │
  ▼
wm.handleEsc(ev, opts)                          [useWindowManager.ts:444]
...
useSimulatorApp processes SSE event
  ...
  │                                                  │
  │ WM: anchor (selectedNodeScreenCenter) может       │
  │ измениться если нода переместилась в layout       │
  │ → watcher fires → wm.open(singleton=reuse)       │
  │   anchorChanged?                                 │
  │     measured=true → НЕ перемещает (keep position) │ [:358]
  │     measured=null → перемещает                    │
  │                                                  │
  │ ВАЖНО: если layout relayout переместил ноду,     │
  │ окно НЕ следует за нодой (by design: user drag)  │
  └──────────────────────────────────────────────────┘
  │
  ├─── Interact panel ──────────────────────────────┐
...
Legacy useNodeCard:                             [useNodeCard.ts:112-113]
  │ cardW = 340  ← HARDCODED
  │ cardH = 240  ← HARDCODED
  │ Реальный размер карточки зависит от контента
  │ (trustlines list, pin actions, interact buttons)
  │ → _reclamp nextTick коррекция компенсирует
...
#### UX-2: Потеря фокуса при переключении/закрытии окон
...
- Focus-trap внутри framed windows (в frameless — optional)
...
#### UX-5: Повторное открытие той же карточки

**Описание:** Двойной клик на ту же ноду при открытой карточке — неопределённое поведение.

**Первопричина:** [`onCanvasDblClick()`](simulator-ui/v2/src/composables/useCanvasInteractions.ts:92) всегда вызывает `setSelectedNodeId(hit.id)` + `setNodeCardOpen(true)`, даже если карточка уже открыта для того же id.

В WM mode:
- `selectNode(id)` → `isNodeCardOpen = false` (сброс!)
- `uiOpenOrUpdateNodeCard` → `wm.open()` singleton=reuse → обновляет data (тот же nodeId)
- watcher: `isNodeCardOpen=false` → пытается закрыть

**Последствия:**
- Мерцание: карточка закрывается и открывается заново
- suppressNextCanvasNodeCardClose может не покрыть этот случай
...
**Вариант исправления:**
- Early return в dblclick handler если `selectedNodeId === hit.id && isNodeCardOpen`
- Или: в WM mode пропускать select/open если окно уже показывает этот nodeId

---

#### UX-6: Быстрые последовательные клики по разным нодам
...
#### ARCH-1: Дуальный рендер-путь (Legacy vs WM)

**Описание:** Два полностью параллельных рендер-пути в template и логике:
- Legacy: `<Transition name="panel-slide">` с `v-if !__USE_WINDOW_MANAGER`
- WM: `<TransitionGroup name="ws">` с `v-if __USE_WINDOW_MANAGER`

**Первопричина:** Эволюционная миграция — WM добавлялся поверх legacy; исторически существовал feature flag для отката. В текущем коде legacy opt-out удалён (WM-only runtime).

**Файлы:**
- [`SimulatorAppRoot.vue`](simulator-ui/v2/src/components/SimulatorAppRoot.vue:1329) — WM layer
- [`SimulatorAppRoot.vue`](simulator-ui/v2/src/components/SimulatorAppRoot.vue:1325) — Legacy transitions
- [`useNodeCard.ts`](simulator-ui/v2/src/composables/useNodeCard.ts:95) — исторически содержал ветвление по `__USE_WINDOW_MANAGER`

**Последствия:**
- Дублирование логики позиционирования
- Двойная стоимость поддержки
- Legacy bugs не фиксятся (но и не проявляются в default mode)
- Тесты должны покрывать оба пути

**Критичность:** P2 (tech debt, но не UX-блокирующий)

**Вариант исправления:**
- Feature flag removal: удалить legacy path после стабилизации WM
- Единый рендер-путь через WM
- Отдельная ветка для legacy fallback (если нужен)
...
isNodeCardOpen changes
  → watcher [581] fires
    → wm.open() / wm.close()
      → policy.onClose → setNodeCardOpen(false)
        → isNodeCardOpen changes
          → watcher [581] fires again
```
...
#### ARCH-5: isNodeCardOpen как legacy-мост

**Описание:** `isNodeCardOpen` — legacy `ref<boolean>` из [`useNodeSelectionAndCardOpen`](simulator-ui/v2/src/composables/useNodeSelectionAndCardOpen.ts:4), но в WM mode фактический источник правды — `windowsMap`.

**Первопричина:** WM был добавлен поверх legacy; `isNodeCardOpen` сохранён для обратной совместимости.

**Файлы:**
- [`useNodeSelectionAndCardOpen.ts`](simulator-ui/v2/src/composables/useNodeSelectionAndCardOpen.ts:4) — `const isNodeCardOpen = ref(false)`
- [`SimulatorAppRoot.vue`](simulator-ui/v2/src/components/SimulatorAppRoot.vue:581) — watcher читает isNodeCardOpen
- [`SimulatorAppRoot.vue`](simulator-ui/v2/src/components/SimulatorAppRoot.vue:600) — onClose sets isNodeCardOpen=false

**Последствия:**
- Два источника правды: `isNodeCardOpen` и `windowsMap.has(nodeCardId)`
- Рассинхронизация: isNodeCardOpen=true, но окно закрыто (или наоборот)
- suppressNextCanvasNodeCardClose — хак для предотвращения рассинхронизации

**Критичность:** P2

**Вариант исправления:**
- В WM mode: `isNodeCardOpen` = computed от `windowsMap`
- Или: удалить `isNodeCardOpen`, весь UI-code читает WM state напрямую
- Step: мигрировать consumers (labelNodes, etc.)

---

#### ARCH-6: Множество ad-hoc ref-флагов для EdgeDetail
...
**Описание:** В legacy mode позиция NodeCard вычисляется дважды:
1. `_baseNodeCardStyle` (computed) — аналитическая позиция
2. `_reclamp` (nextTick) — коррекция по getBoundingClientRect

**Первопричина:** [`useNodeCard.ts`](simulator-ui/v2/src/composables/useNodeCard.ts:243) — watch на `_baseNodeCardStyle` вызывает nextTick → BRC.

**Файлы:**
- [`useNodeCard.ts`](simulator-ui/v2/src/composables/useNodeCard.ts:94) — `_baseNodeCardStyle` computed
- [`useNodeCard.ts`](simulator-ui/v2/src/composables/useNodeCard.ts:243) — watch + nextTick + BRC

**Последствия:**
- Forced layout (BRC) на каждое изменение позиции
- Visible "jump": первый кадр с base position, второй с correction
- При pan/zoom: множественные BRC вызовы

**Критичность:** P3 (legacy only, не применяется в WM mode [:246])

**Вариант исправления:**
- Удалить legacy path при полной миграции на WM
- Или: всё позиционирование через одну аналитическую формулу, без BRC

---

#### PERF-2: ResizeObserver → updateMeasuredSize → reclamp цепочка
...
#### PERF-4: Hardcoded размеры vs dynamic content
...
Legacy [`useNodeCard.ts`](simulator-ui/v2/src/composables/useNodeCard.ts:112): `cardW=340, cardH=240` — ещё более грубое приближение.
...
#### RACE-1: Rapid node clicks → watcher coalescing
...
- Watcher [`[:581]`](simulator-ui/v2/src/components/SimulatorAppRoot.vue:581): async trigger на `isNodeCardOpen` change
...
- [`SimulatorAppRoot.vue`](simulator-ui/v2/src/components/SimulatorAppRoot.vue:581) — NodeCard watcher

**Последствия:**
- A→B→C rapid clicks: 3 вызова wm.open(), но watcher может fire только для последнего
- Промежуточные состояния (nodeId=A, anchor=B) — transient, но могут flash
...
- AbortController для HTTP-запросов, привязанный к epoch
...
### 4.4 Hardcoded cardW/cardH в useNodeCard

[`useNodeCard.ts`](simulator-ui/v2/src/composables/useNodeCard.ts:112):
```ts
const cardW = 340
const cardH = 240
```

Эти значения используются ТОЛЬКО в legacy mode для 4-direction placement. В WM mode [`_baseNodeCardStyle`](simulator-ui/v2/src/composables/useNodeCard.ts:95) просто возвращает `{display: 'block'}`.

**Проблема:** В legacy mode:
- Реальная высота NodeCard в interact mode ~380px (кнопки + trustlines list)
- placement алгоритм считает cardH=240 → выбирает "bottom" placement
- _reclamp correction обнаруживает что карточка вылезла → сдвигает вверх
- Двойной скачок: "поставить снизу" → "сдвинуть вверх"
...
1. **Создать `useWindowController()`**
   - Input: interactPhase, selectedNode, isNodeCardOpen, etc.
   - Output: desiredWindows computed
   - Один watch: `desiredWindows → applyDiffToWM()`

2. **Убрать множественные watchers из SimulatorAppRoot**
   - Заменить watchers [:465], [:513], [:581] на один watch в controller

3. **Переместить `makeInteractPanelWindowData` в controller**

#### Этап 4: Устранение legacy path (ARCH-1, ARCH-5)

1. **Удалить legacy opt-out feature flag**
   - Убрать `__USE_WINDOW_MANAGER` checks
   - Убрать `handleEscOverlayStack`
   - Убрать legacy `<Transition name="panel-slide">`

2. **Удалить `isNodeCardOpen` ref**
   - Заменить на computed от WM state
   - Мигрировать consumers (useLabelNodes, etc.)

3. **Удалить legacy positioning из useNodeCard**
   - Убрать 4-direction placement, _reclamp
   - Оставить только `selectedNodeScreenCenter` для anchor
...
| # | Критерий | Измерение | Target |
|---|---------|-----------|--------|
| A-1 | SimulatorAppRoot.vue < 800 строк | `wc -l` | Confirmed |
...
| A-5 | Нет `isNodeCardOpen` ref (после этапа 4) | Code search | 0 references |
...
| [`SimulatorAppRoot.vue`](simulator-ui/v2/src/components/SimulatorAppRoot.vue:1) | 1600+ | Root: watchers, ESC, template, bridging |
| [`useNodeCard.ts`](simulator-ui/v2/src/composables/useNodeCard.ts:1) | 276 | Legacy positioning + WM bypass |
| [`useNodeSelectionAndCardOpen.ts`](simulator-ui/v2/src/composables/useNodeSelectionAndCardOpen.ts:1) | 25 | selectNode + isNodeCardOpen ref |
...
| UX-2 | UX/A11y | P2 | Этап 5 | Потеря фокуса при закрытии | — |
...
| UX-5 | UX | P3 | Этап 3 | Повторное открытие той же карточки | — |
| UX-6 | UX | P3 | Этап 3 | Быстрые последовательные клики | — |
...
| **UX-9** | **UX** | **P2** | **Этап 1** | **NodeCard anchor drift при pan/zoom** | **🆕 v2** |
...
| ARCH-5 | Arch | P2 | Этап 4 | isNodeCardOpen legacy bridge | — |
| ARCH-6 | Arch | P2 | Этап 2 | Множество ad-hoc EdgeDetail flags | — |
| ARCH-7 | Arch/UX | **P1** | Этап 2 | keepAlive EdgeDetail context drift | ⬆️ P2→P1 |
| PERF-1 | Perf | P3 | Этап 4 | Двухпроходное legacy positioning | — |
| PERF-2 | Perf | P3 | Этап 5 | RO → reclamp chain | — |
| PERF-3 | Perf | P3 | Этап 3 | Множественные watchers | — |
| PERF-4 | Perf | P2 | Этап 5 | Hardcoded sizes vs content | — |
| RACE-1 | Race | P3 | Этап 3 | Rapid node clicks | — |
| RACE-2 | Race | P2 | Этап 1 | ESC during async | — |
| RACE-3 | Race | P3 | Этап 3 | Concurrent wm.open | — |
| RACE-4 | Race | P2 | Этап 1 | Stale epoch in runBusy | — |
...
|----|-----------|--------|
| WM-1 | WM единственный source of truth | ⚠️ Partially (isNodeCardOpen bridge) |
| WM-2 | Group limits: max 1 per group, coexistence | ✅ |
| WM-3 | ESC back-stack + form-guard | ✅ |
| WM-4 | Canvas click = hard dismiss | ⚠️ Too aggressive (UX-4) |
...
|------|------|---------|-----------|
| Global ESC dispatch не локализован | useWindowManager.ts | Custom event goes to `window`, not per-window | P3 |
| A11y: focus-trap, focus-return | — | Не формализован | P2 |
| Bottom-sheet для узких экранов | — | POST-MVP | P4 |
| Удалить legacy ESC stack | SimulatorAppRoot.vue | В рамках WM-only runtime | P2 (Этап 4) |
