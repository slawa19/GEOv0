...
> **✅ ARCHIVED — 2026-02-28**
> Все фазы реализованы и покрыты тестами. DoD закрыт по всем автоматизируемым пунктам (P0/P1/P2 замечания устранены). Ручная визуальная проверка остаётся опциональной (§14.3).
> Код-ревью: [code-review report 2026-02-28](../../../../../../../plans/manual-operations-ui-code-review-2026-02-28.md) — не создавался; результаты зафиксированы в §14 этого документа.

Статус: ~~draft v2~~ **DONE** (все фазы 1/2/2.5/3 реализованы; все AC-тесты закрыты; архивировано 2026-02-28)
...
## 14. Consolidated TODO (updated 2026-02-28)
...
### 14.2 ~~TODO~~ **DONE**: Недостающие тесты по AC-идентификаторам

> **Статус (2026-02-28):** Все T-1..T-6 выполнены в коде; AC-MP-13 добавлен как новый тест-файл.

Все тест-кейсы найдены/добавлены:

| # | AC | Статус | Файл : строка |
|---|-----|--------|---------------|
| T-1 | **AC-ED-5** | ✅ DONE | [`EdgeDetailPopup.test.ts:123`](simulator-ui/v2/src/components/EdgeDetailPopup.test.ts) |
| T-2 | **AC-MP-11** (×3) | ✅ DONE | [`ManualPaymentPanel.test.ts:843,913,1043`](simulator-ui/v2/src/components/ManualPaymentPanel.test.ts) |
| T-3 | **AC-MP-12** | ✅ DONE | [`ManualPaymentPanel.test.ts:104`](simulator-ui/v2/src/components/ManualPaymentPanel.test.ts) |
| T-4 | **AC-TL-10** | ✅ DONE | [`TrustlineManagementPanel.test.ts:156`](simulator-ui/v2/src/components/TrustlineManagementPanel.test.ts) |
| T-5 | **AC-A11Y-1/2** | ✅ DONE | [`ManualPaymentPanel.test.ts:38`](simulator-ui/v2/src/components/ManualPaymentPanel.test.ts) |
| T-6 | **AC-A11Y-3** | ✅ DONE | [`SuccessToast.test.ts:7`](simulator-ui/v2/src/components/SuccessToast.test.ts) — тест переименован + добавлено `aria-live` assertion |
| T-7 | **AC-MP-13** (новый) | ✅ DONE | [`useInteractMode.startPaymentFlow.test.ts`](simulator-ui/v2/src/composables/useInteractMode.startPaymentFlow.test.ts) — spy на `refreshTrustlines({ force: true })` |
...
### 14.5 ~~TODO~~ **DONE**: `toSelectionInvalidWarning` не сбрасывается при canvas-driven From change

**Статус (2026-02-28):** РЕАЛИЗОВАНО. Watcher добавлен в [`ManualPaymentPanel.vue`](simulator-ui/v2/src/components/ManualPaymentPanel.vue) — помечен комментарием `// UX-14.5`.

```typescript
// UX-14.5 — сброс при canvas-driven смене From (минует onFromChange)
...
### 14.6 ~~TODO~~ **DONE**: ED-3 — contextual button label

**Статус (2026-02-28):** РЕАЛИЗОВАНО. Кнопка uses computed `sendPaymentFromLabel = "💸 Pay {fromPid}"`
в [`EdgeDetailPopup.vue`](simulator-ui/v2/src/components/EdgeDetailPopup.vue).
Direction confusion устранён: `state.fromPid` содержит payer pid (reversed trustline direction).
...
### 14.7 ~~TODO~~ **DONE**: `reverse_used` в snapshot fallback

**Статус (2026-02-28):** РЕАЛИЗОВАНО. Маппинг `reverse_used` из `snapshot.links` добавлен
в `useInteractDataCache.ts`; покрыто тестом в
[`useInteractDataCache.snapshotTrustlines.test.ts:37`](simulator-ui/v2/src/composables/interact/useInteractDataCache.snapshotTrustlines.test.ts):
- «maps reverse_used from snapshot.links when present (14.7)» — ✅
- «does not invent reverse_used when snapshot.links does not have it (known limitation)» — ✅
...
### 14.9 ~~TODO~~ **DONE**: Phase 3 DoD — integration-тесты для ED-3

**Статус (2026-02-28):** РЕАЛИЗОВАНО. Тест `AC-ED-3` добавлен в
[`SimulatorAppRoot.interact.test.ts:591`](simulator-ui/v2/src/components/SimulatorAppRoot.interact.test.ts).
Проверяет: кнопка `[data-testid="edge-send-payment"]` → `startPaymentFlow` → direction reversal
(trustline `alice→bob` → payment `bob→alice`) → pre-fill From/To → confirm step рендерится.
...
|---|--------|-----------|--------|---------------|
| 14.5 | `toSelectionInvalidWarning` reset при canvas-click | ~~Medium~~ **DONE** | — | — |
| 14.2 | Тесты по AC (6+1 шт.) | ~~Medium~~ **DONE** | — | — |
| 14.9 | Integration-тест ED-3 | ~~Medium~~ **DONE** | — | — |
| 14.6 | ED-3 contextual label | ~~Low~~ **DONE** | — | — |
| 14.7 | `reverse_used` snapshot fallback | ~~Low~~ **DONE** | — | — |
| 14.3 | Визуальная проверка Phase 1 + 2 | **Medium** | XS (manual) | Phase 1+2 DoD ✅ |
| 14.4 | E2E тесты (Playwright, E-1..E-4) | **Medium** | M–L | Нет (nice-to-have) |
