...
3) **FX overlay** (второй canvas поверх графа): искры Tx, клиринг по маршруту/циклу, вспышки/кольца. **Статус:** ✅ DONE (2026-01-22) — выделен отдельный overlay-canvas и реализованы базовые эффекты: Tx comet sparks, clearing edge pulses + node bursts + flash. В `VITE_TEST_MODE=1` FX отключены для стабильных скриншотов.
