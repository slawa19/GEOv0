...
3) **FX overlay** (второй canvas поверх графа): искры Tx, клиринг по маршруту/циклу, вспышки/кольца. **Статус:** ✅ DONE (2026-01-22) — выделен отдельный overlay-canvas и реализованы базовые эффекты: Tx comet sparks, clearing edge pulses + node bursts + flash. Для стабильных скриншотов FX должны быть детерминированы: в `VITE_TEST_MODE=1` под WebDriver/Playwright эффекты не спавним (допускаем оставить только статические подсветки рёбер).
...
4) **Синхронизация в Admin UI + валидация (чтобы сравнивать демо с interactive mode)**
...
- можно было сравнивать поведение визуала с Admin UI interactive mode на тех же данных.
...
Остальные эквиваленты подключаются только в **interactive mode** (будущий этап).
...
В `test-mode` (для e2e/visual под Playwright/WebDriver) UI должен:
...
Примечание: в интерактивном запуске (без WebDriver) допускается оставлять FX включёнными даже при `VITE_TEST_MODE=1` — важнее сохранить удобство ручного дебага, а детерминизм обязателен именно для скриншотов.
...
- Во всех сценах (A–E) узлы соответствуют screen-prototypes:
  - `business` — emerald square, `person` — blue circle;
  - balance glow появляется только на focused node (credit=cayn, debt=orange, near-zero=neutral);
  - базовый цвет и размер узла строго следуют `viz_color_key` и `viz_size` (клиент не вычисляет семантику);
  - эффекты/тайминги детерминированы (никакого `Math.random()`), чтобы визуал утверждался воспроизводимо.
