...
1) **Данные: fixtures-first источник правды**
   - Сделать загрузку snapshot/events только из `public/simulator-fixtures/...` (без backend).
...
Базовый snapshot (общий для всех сцен):
- `simulator-ui/v2/public/simulator-fixtures/v1/HOUR/snapshot.json`

Плейлисты событий:
- Scene D (Tx burst): `simulator-ui/v2/public/simulator-fixtures/v1/HOUR/events/demo-tx.json`
- Scene E (Clearing plan): `simulator-ui/v2/public/simulator-fixtures/v1/HOUR/events/demo-clearing.json`
...
2) Положить snapshot в `simulator-ui/v2/public/simulator-fixtures/v1/HOUR/snapshot.json`.
