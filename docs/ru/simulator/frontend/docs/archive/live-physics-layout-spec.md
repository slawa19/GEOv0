...
Дополнительно для стабильности UI и скриншотов:

- В среде WebDriver (`navigator.webdriver===true`, Playwright) некоторые чисто-визуальные эффекты отключаются/фиксируются (например, glow у всплывающих сумм, масштабирование DOM-оверлеев).
...
## 5.1. UI overlays: подписи узлов и всплывающие суммы

В v2 часть текста рендерится DOM-оверлеями поверх canvas (для простоты и читабельности).

### Подписи узлов (node labels)

- Показываются только в runtime (в `test-mode` выключены).
- **Позиционирование:** размещаются **ниже узла**.
  - Смещение задаётся в px (screen-space) и переводится в world-units через `camera.zoom`, чтобы расстояние было стабильным визуально.
- **Масштабирование:** подписи не используют `worldToCssTranslate()` (там есть `scale(1/zoom)`), вместо этого используется translate без scale.
- **Стиль:** центрирование по тексту, **без рамки/пилла**, мягкое серо‑чёрное “притенение” (glow/shadow), чтобы подпись читалась на фоне.

### Всплывающие суммы (floating amounts)

- Базово “воздушные” (лёгкий halo + текущий цвет).
- **Усиление читабельности:** включается **только когда есть риск слияния с узлом** — мягкое сияние (glow) включается по детерминированному геометрическому триггеру (расстояние до ближайшего узла в screen-space).
- В WebDriver-run (Playwright) glow принудительно выключается для стабильных скриншотов.

### Масштабирование оверлеев на zoom-out

- На малом увеличении (zoom-out) подписи должны “удаляться” вместе с графом — визуально уменьшаться.
- Реализация: CSS-переменная `--overlay-scale = clamp(camera.zoom, min, 1)`.
  - Рекомендованный диапазон: `min≈0.65`, `max=1` (подстройка по UX).
- В `test-mode` и WebDriver `--overlay-scale=1` (стабильность).
...
### Фаза 6: UX overlays (итеративно)
- [x] Всплывающие суммы: glow/сияние включается только при риске слияния с узлом (геометрический триггер), в WebDriver выключено
- [x] Подписи узлов: центрирование, без рамки, серое притенение, размещение ниже узла
- [x] Zoom-out: оверлеи (подписи/суммы) визуально уменьшаются с ограничением min/max; в test-mode/WebDriver фиксированы
...
Дополнение (визуальные эффекты):

- Риск: “плавающие” визуальные эффекты (glow/FX) могут давать скриншот‑дифы в Playwright.
- Митигация: в WebDriver эффекты отключаются/фиксируются (не влияет на runtime UX).
...
7. ✅ При zoom-out подписи (DOM overlays) визуально уменьшаются в пределах min/max
8. ✅ Glow у всплывающих сумм проявляется только при риске слияния с узлом
