...
### 3.5 Window lifecycle vs focus ownership

Для WM-managed windows lifecycle и focus ownership считаются разными измерениями и не должны сливаться в один state field.

Правила:

- `lifecyclePhase` описывает только render/measurement path: `mounting -> measuring -> stable -> closing`;
- `active` описывает только текущего focus/topmost owner среди focus-eligible WM windows;
- окно в `closing` не может оставаться `active` и не может быть target для нового focus raise;
- non-WM runtime surfaces не обязаны получать отдельный instance lifecycle field, если для них уже достаточно family descriptor contract и owner-specific runtime logic.

Следствие:

- `active` не должен использоваться как синоним `stable` или `mounted`;
- `lifecyclePhase` не должен кодировать focus semantics вроде `active` / `inactive`.
...
Дополнительный guardrail для overlap routing:

5. если change затрагивает pointer-transparency или wheel-routing между passive overlay и canvas, одного unit/integration proof недостаточно;
6. в таком случае нужен browser-level regression (например Playwright) с реальным hit-testing proof через overlap scenario;
7. минимальный acceptable proof: passive overlay в overlap-точке не становится top hit target вместо canvas, а interactive WM shell в overlap-точке остаётся top hit target.
