...
**baseline** на эквивалент; без него результат — `UNVERIFIABLE`.

- **Свежая локальная база — автоматически.** `scripts/run_local.ps1` (`start` без базы и `reset-db`),
  `scripts/run_full_stack.ps1` (без базы и `-ResetDb`) и `scripts/verify_admin_phase4_real_contract.ps1`
  снимают его сразу после заливки, до старта бэкенда, и падают, если он не снялся. После ручной заливки
  выполните команду выше сами, до платежей.
- **Обновление существующей базы — явно и вручную (переключение).** Сначала сделайте систему тихой:
  остановите клиентский трафик и прогоны симулятора, а также всё, что пишет `debts` в обход приложения
  (сервисный SQL, восстановление из копии, скрипт мимо журнала долгов). Запущенный, но простаивающий API
  писателем не является и может остаться: платёж или клиринг, если они всё же случатся, берут owner-lock
  эквивалента, который берёт и baseline, а заливка, гонящаяся с baseline, отвергается на уровне
  изоляции приложения `SERIALIZABLE` (`40001`). Затем:
  `docker compose exec app python scripts/take_reconciliation_baseline.py --all` или `--equivalent CODE`
  (можно повторять).
  - `--all` снимает baseline для каждого эквивалента, у которого его ещё нет, и пропускает те, у которых
    он уже есть. `--equivalent CODE` отказывает эквиваленту, у которого baseline уже есть (код выхода 1).
  - Каждый эквивалент — одна транзакция под его owner-lock.
  - Всё, что журнал в этот момент не объясняет, — например долги, записанные до появления журнала, —
    записывается смещением по ребру. **Baseline не удостоверяет эти долги**, он лишь делает проверяемыми
    последующие изменения.
  - **Перебазирования нет.** На эквивалент ровно один baseline.
...
### 6.6 Контейнеры остановились сами, хотя ничего не падало

Симптом: `docker ps -a` показывает все три контейнера `Exited (0)` с одним и тем же временем,
в логах ошибок нет. Причина не в Compose и не в приложении: WSL останавливает по простою сам
дистрибутив, а `restart` в dev-override намеренно `"no"` (fail fast), поэтому контейнеры не
возвращаются.

Проверка — внутри дистрибутива:
- `uptime -s` — если время старта изменилось, дистрибутив (а с ним и Docker) перезапускался.

Лечение — в `%UserProfile%\.wslconfig`:
...
`instanceIdleTimeout` — таймер простоя **дистрибутива**, по умолчанию 15000 мс; `-1`
документирован как отключение авто-остановки (WSL 2.4.4+, см.
[Advanced settings configuration in WSL](https://learn.microsoft.com/en-us/windows/wsl/wsl-config)).
Не путайте с `[wsl2] vmIdleTimeout`: тот относится к виртуальной машине и начинает считать
только после того, как остановлены все дистрибутивы, — контейнеры им не спасти. Настройка
применяется только после `wsl --shutdown`.

Проверено 2026-09-20 на WSL 2.7.14: без этого ключа стенд не переживал 30 секунд простоя,
с ним — 9 минут простоя при неизменном `uptime -s` и всех контейнерах `healthy`.

Поднять стенд после остановки:
