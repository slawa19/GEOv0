...
Примечание (реализация Hub v0.1): промежуточное состояние `PREPARE_IN_PROGRESS` в БД не фиксируется. После успешного `prepare_routes(...)` транзакция переводится в `PREPARED`, после чего выполняется `COMMIT`.
...
Примечание (реализация Hub v0.1): в БД используются минимально необходимые состояния `NEW → PREPARED → COMMITTED/ABORTED`.
Состояния `ROUTED` и `PREPARE_IN_PROGRESS` зарезервированы для расширенного 2PC/межузлового режима, но в текущем MVP не используются как устойчивые состояния.
