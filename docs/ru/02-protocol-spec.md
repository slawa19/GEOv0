...
- `metadata.type` — одно из `fiat | time | commodity | custom`
- `metadata.iso_code` — опционально; допускается только при `metadata.type == fiat` и должен быть в формате `^[A-Z]{3}$` (например, `UAH`)
...
| `daily_limit` | Лимит оборота в сутки (**в MVP не enforced; informational only**) | `null` (без лимита) |
...
Важно: в текущей реализации Hub (MVP v0.1) полевая модель «операция протокола» и внутренняя модель БД разведены:

- Таблица `transactions` используется как **внутренний журнал** прежде всего для `PAYMENT` и `CLEARING`.
- Операции управления trustline (`TRUST_LINE_CREATE/UPDATE/CLOSE`) в текущем MVP **не создают запись** в `transactions`; вместо этого фиксируются в audit-log (операторский след и контроль целостности).
- В реализации могут присутствовать **дополнительные internal-only типы** транзакций (например, `COMPENSATION`, `COMMODITY_REDEMPTION`), которые не являются частью wire-протокола.
- Если в будущем потребуется унификация аудита через `Transaction` для этих операций, целевой поток должен быть **single-phase**: `NEW → COMMITTED/ABORTED` (без 2PC), чтобы не усложнять там, где не нужны prepare-locks и recovery для мульти-ребёрных эффектов.
...
5. (Hub v0.1) Зафиксировать запись в `IntegrityAuditLog` (best-effort). `tx_id` может быть `null`.
...
2. Обновить запись TrustLine
3. (Hub v0.1) Зафиксировать запись в `IntegrityAuditLog` (best-effort). `tx_id` может быть `null`.
...
4. (Hub v0.1) Зафиксировать запись в `IntegrityAuditLog` (best-effort). `tx_id` может быть `null`.
...
Примечание (реализация Hub v0.1): промежуточные состояния в БД не фиксируются. С 2026-09-25 (программа 019, стадия 4) multi-path платёж, как и single-path, исполняется одной транзакцией: все маршруты связываются и применяются вместе, строка вставляется `COMMITTED` (или `ABORTED` при окончательном отказе) — см. примечание к 6.8. *Историческое:* до этой даты после `prepare_routes(...)` транзакция переводилась в `PREPARED`, затем выполнялся `COMMIT`.
...
Примечание (реализация Hub v0.1, **с 2026-09-25**, программа 019, стадия 4): хаб исполняет REST-платёж (`POST /payments`) **одной транзакцией БД** и **не сохраняет** промежуточных состояний: строка платежа вставляется сразу `COMMITTED` вместе с деньгами (конвертом операции, записями журнала, долгами и строкой аудита) либо `ABORTED` для окончательного отказа; неудача, после которой транзакция откатана, не оставляет строки. `NEW` и `PREPARED` для `PAYMENT` хабом не записываются, и миграция `030` это закрепляет ограничением `type <> 'PAYMENT' OR state IN ('COMMITTED','ABORTED')` (ограда устаревшей записи платежа; клиринг и прочие типы вне её). Диаграмма выше описывает протокольную стейт-машину **сетевого** режима: `PREPARED` и резервирование участников (6.5.3) остаются описанием зарезервированных сценариев — сетевого `PAYMENT_PREPARE_ACK` и межхабового 2PC (9.5, 8.6), — а не того, что хаб хранит для REST-платежа. Публичный статус платежа, как и прежде, — только `COMMITTED`/`ABORTED`.

*Историческое (до 2026-09-25):* в БД фиксировались `NEW → PREPARED → COMMITTED/ABORTED` с резервами `prepare_locks` между коммитами; `ROUTED` и `PREPARE_IN_PROGRESS` не использовались как устойчивые состояния.
...
Конкурентные ошибки PostgreSQL `40001`/`40P01` сначала обрабатываются whole-UoW retry. Если
конфликт достигает границы сервиса, публичный ответ — существующий `409/E008` с безопасным
признаком `details.retryable=true`, а не `500/E010`; диагностический текст драйвера не раскрывается.

Timeout и отмена запроса не оставляют устойчивые `NEW`/`PREPARED`: cleanup дренируется до
завершения до закрытия владеющей session. При неоднозначности после COMMIT сначала перечитывается
состояние; `COMMITTED` имеет приоритет и не регрессирует в `ABORTED`.
...
(Зарезервировано; не реализовано в Hub v0.1.)

Если хотя бы у одного `policy.auto_clearing = false` (будущая версия):
...
#### 7.6.1. Конкурентная граница платежей и клиринга

Примечание (реализация Hub v0.1; заменено 2026-09-25, программа 019, стадия 5, `T1909`). У
REST-платежей **нет долговечных резервов** (таблица `prepare_locks` удалена миграцией `031`): платёж —
одна транзакция без промежуточных состояний, видимых другим транзакциям. Граница с клирингом:

- **Изоляция и повторы.** Конкурентные денежные операции (платёж, staged-фаза симулятора, инжект,
  клиринг) исполняются в `SERIALIZABLE` (уровень рабочей транзакции проверяется на границе) и при
  конфликте повторяют **всю** транзакцию на свежем снимке в ограниченном бюджете; повтор из того же
  снимка (откат savepoint'а) повтором не считается. Конфликт — `40001`, `40P01` и `23505` ровно на
  уникальности строки долга (две конкурентные вставки одной новой пары).
- **Одна advisory-идентичность эквивалента в двух режимах.** Платёж, staged-фаза и инжект держат её
  **разделяемой** (друг друга не ждут); клиринг — **исключительной**. Staged-операция берёт полный набор
  эквивалентов в одном глобальном порядке до защищаемых записей.
- **Свежий снимок клиринга.** Клиринг берёт исключительный лок на закреплённом соединении **до** своего
  авторитетного снимка (ожидает завершения разделяемых держателей), откатывает транзакцию захвата и
  только затем читает; лок держится через разрешение коммита и все разрешённые повторы.
- **Стоп/hold.** Каждый денежный писатель проверяет стоп/hold через применимую границу блокировки строки
  эквивалента (`FOR SHARE`); клиринг держит `FOR SHARE` до своего денежного коммита. Операции оператора
  (деактивация, hold, удаление эквивалента) изменяют эту строку и потому ждут писателей, уже прочитавших
  её, а писатель, читающий после них, получает конфликт и на повторе — отказ.
- **Строки долга** перечитываются в каждой попытке (`FOR UPDATE` цикла у клиринга).
- **Исход.** Подтверждённая occurrence клиринга (стабильный идентификатор) разрешает повтор; неизвестный
  исход коммита вслепую не повторяется. Отмена после коммита сохраняет фактически закоммиченную сумму
  клиринга.
- **Очистка сессионного лока** требует подтверждённого освобождения; если освобождение не подтверждено,
  соединение инвалидируется и физически закрывается, а не возвращается в pool.
- Advisory-идентичность не меняет направление Debt, TrustLine, flow или audit payload.

Смешанные версии не поддерживаются: развёртывание и откат (`031` и обратно) требуют согласованной
остановки всех денежных писателей (`docs/ru/05-deployment.md`).
...
Примечание (реализация Hub v0.1): клиринг выполняется атомарно в одной БД-транзакции.
Состояния `PROPOSED`, `WAITING`, `REJECTED` зарезервированы для расширенного режима с явным согласием,
но в текущем MVP не используются как устойчивые состояния.
...
> **Реализация Hub v0.1 с 2026-09-25 (программа 019, стадия 4).** Восстанавливать после сбоя хабу нечего: REST-платёж — одна транзакция БД, и сбой процесса либо откатывает её целиком (строки нет, повтор того же `tx_id` исполняется заново), либо застаёт её уже закоммиченной (повтор получает сохранённый результат, 9.1). Незавершённых платежей в БД не бывает — миграция `030` запрещает нетерминальное состояние `PAYMENT`, — поэтому фоновый цикл восстановления и жнец резервов (`app/core/recovery.py`) удалены вместе с запуском. Переход на эту реализацию требует осушения базы старой версией (`docs/ru/05-deployment.md`, перевод для миграции `030`). Псевдокод 9.3.1–9.3.2 ниже описывает восстановление **сетевого/межхабового** режима с долговечными промежуточными состояниями (зарезервировано, 9.5) и **исторически** — поведение хаба до этой даты.
...
*Историческое / сетевой режим (см. примечание к 9.3):* хаб с 2026-09-25 резервов `prepare_locks` для платежа не пишет; таблица и её читатели (маршрутизация, исключение пар в клиринге) остаются до стадии 5 программы 019 (`T1909`, миграция `031`).
...
### 9.5. Онлайн/оффлайн и «асинхронность» (практическая интерпретация)

В документации встречаются два разных смысла «асинхронности»:

1) **Клиентский уровень (UX):** пользователь/клиент может быть оффлайн, а уведомления доставляются best‑effort (например, WebSocket). После восстановления связи клиент **всегда** сверяет состояние через REST.

2) **Протокольный уровень (согласования/ACK):** некоторые протокольные сценарии требуют **ответов** от участников или соседних hub'ов (ACK/голоса) в пределах дедлайна (`timeout_at`/`expires_at`). Именно здесь появляются состояния вроде `PREPARE_IN_PROGRESS`, `PROPOSED`, `WAITING`, `REJECTED`.

Ниже — минимальная матрица требований к онлайности по сценариям.

| Сценарий | Требуются ответы участников (онлайн) | Что происходит при оффлайне/таймауте | Статус в Hub v0.1 |
|---|---|---|---|
| **PAYMENT (hub‑centric REST)** | **Нет**: получатель может быть оффлайн; промежуточные «узлы» не являются сетевыми участниками протокола | Операция завершается на Hub; получатель узнаёт позже через WebSocket/REST | **Реализовано** |
| **PAYMENT с сетевым PREPARE_ACK** (`PAYMENT_PREPARE_ACK`) | **Да**: каждый участник маршрута должен подтвердить PREPARE | Таймаут → ABORT; возможен последующий recovery | **Зарезервировано** (см. примечания к состояниям в 6.8) |
| **CLEARING auto** (`policy.auto_clearing=true` по всем рёбрам) | **Нет**: явные подтверждения не требуются | Hub применяет клиринг атомарно; участники узнают пост‑фактум | **Реализовано** |
| **CLEARING explicit consent** (`CLEARING_ACCEPT/REJECT`) | **Да**: требуется явное согласие всех участников | Любой `REJECT` или истечение `expires_at` → `REJECTED`/ABORT | **Зарезервировано** (см. 7.4.2 и 7.7) |
| **Inter‑hub payment/2PC** | **Да**: оба hub'а должны обменяться сообщениями и синхронизировать фиксацию | Таймауты/ретраи; recovery/эскалация при неопределённости | **Частично описано протокольно**, не является MVP‑требованием |

Примечание: раздел 9.4 (recovery) описывает протокольный механизм «eventual consistency» для распределённого режима. В hub‑centric MVP основная практическая защита от оффлайна клиента — это **идемпотентность** и возможность повторного запроса результата операции.
...
#### 11.2.1. Zero-Sum: тождество модели, а не проверка

**Статус: снято с публикации как пройденная проверка 2026-09-11** (программа 014, `T1402`).
Раздел сохранён, потому что само свойство верно и его стоит понимать — но понимать правильно.
...
**Это тождество рёберной модели, а не наблюдение о состоянии базы.** Долг хранится одним ребром
`Debt(debtor, creditor, amount)`, и net-позиция считается как «кредиты минус долги». Значит каждое
ребро вносит `+amount` своему кредитору и `−amount` своему должнику — **одно и то же число с двумя
знаками**. Сумма по всем участникам телескопируется в ноль при **любом** наборе рёбер с конечными
суммами. Не «должна равняться нулю, иначе что-то сломано», а «равна нулю по построению».

**Отсюда следствие, ради которого этот раздел переписан:** запрос, вычисляющий эту сумму, **не
способен обнаружить повреждение данных**. Измерено на PostgreSQL 2026-09-11: раздутие одного долга
на один квант хранения (`0.00000001`) и равномерное раздутие всех трёх рёбер замкнутого цикла на
`7.00000000` — оба оставляют проверку пройденной; во втором случае неизменными остаются и
нетто-позиции. Прежняя редакция этого раздела приводила SQL-запрос именно такой формы и называла его
«SQL-проверкой». Запрос был верен; слово «проверка» — нет.

**Что делает система сегодня.** `zero_sum` присутствует в ответах `/api/v1/integrity/status`,
`/verify` и в чекпойнтах, но **вердикта не несёт**: `{"status": "not_verified", "reason":
"check_withdrawn"}`. Поля `passed` у него нет и схемой оно запрещено, чтобы «не проверяется» нельзя
было прочитать как «пройдено». Совокупный `status` считается только по фактически выполненным
проверкам, а список невыполненных публикуется в `unverified` — иначе ложная уверенность просто
переехала бы из строки в сводку. Вызов с платёжного пути снят: он сканировал весь эквивалент на
каждом платеже и не мог ничего найти.

**Что обнаруживает повреждение на самом деле.** Расхождение между `debts` и журналом операций, то
есть попарная сверка рёбер с историей, которая их породила. Она сделана программой 015
(`app/core/ledger/reconciliation.py`): критерий (а) — по каждому ребру `debts` минус сумма
записанных `delta` равен смещению baseline, критерий (б) — записанное изменение равно записанному
намерению операции; сверка идёт по расписанию после коммита, подтверждённый `FAILED` ставит удержание
эквивалента. Журнал с программы 018 (миграция `029`) пишет сама база: триггер на `debts` строит
запись из `OLD`/`NEW` и отвергает запись долга вне открытой операции. Это **обнаружение**, а не
защита от подделки: согласованная правка долга, журнала и намерения, отключённый триггер и неверный
baseline сверкой не ловятся (перечень — в `reconciliation.py`). `zero_sum` в ответах integrity
по-прежнему вердикта не несёт — результат сверки хранится отдельно (`debt_reconciliation_results`).

**Исторические записи не переписаны.** Строки аудита, сохранённые до снятия, несут
`zero_sum: {"passed": true}` — это свидетельство о том, что система утверждала тогда, и читать его
как текущую верификацию нельзя.

**Частота проверки:** не применяется — проверка не выполняется.
...
LEFT JOIN trust_lines tl ON 
    ...
    -- frozen сверяется с сохранённым лимитом (§3.3, §11.5.2); closed или отсутствующая линия — лимит 0
...
### A. Канонический JSON (normative)

Канонизация нужна для того, чтобы разные реализации (клиент/сервер, разные языки) получали байтово-идентичный `message` для подписи.

**Общее правило:** подписываемый `message` — это сериализация canonical JSON от payload/объекта, из которого исключены поля подписи (например, `signatures`).

**Требования к канонизации:**
1. **Кодировка:** UTF-8
2. **Порядок ключей:** ключи объектов сортируются лексикографически (по Unicode code points) и сериализуются в этом порядке
3. **Разделители:** без пробелов (минимальная JSON запись)
4. **Строки:** стандартный JSON escaping; запрещены альтернативные эквивалентные формы (например, разные escape-последовательности для одного символа, где это применимо)
5. **Числа:**
   - запрещены `NaN`, `Infinity`, `-Infinity`
   - без ведущих `+`, без ведущих нулей (кроме числа `0`)
   - без “лишних” нулей в дробной части (например, `1.2300` недопустимо)
   - экспоненциальная форма должна быть либо запрещена, либо определена явно (рекомендуется запретить для простоты)
6. **null/boolean:** стандартные JSON `null`, `true`, `false` (в нижнем регистре)

**Примечание:** вместо собственного описания можно принять RFC 8785 (JSON Canonicalization Scheme) как норматив и ссылаться на него; тогда реализация должна соответствовать RFC.
...
`tx_id` — идентификатор операции/транзакции для идемпотентности и дедупликации.

**Базовая норма (рекомендуется для v0.1):**
- `tx_id` генерируется инициатором операции (клиентом) и повторно используется при ретраях
- формат: UUID v4 (или UUID v7, если выбран как стандарт проекта)
- для каждой новой операции должен быть новый `tx_id`

**Idempotency / конфликт payload:**
- `same tx_id` + `same canonical payload` → сервер возвращает тот же результат (идемпотентный ответ), повторно эффекты не применяются
- `same tx_id` + `different canonical payload` → сервер возвращает ошибку конфликта (например, 409) и **не** применяет изменения

**Hash-вариант (если допускается):**
- если `tx_id` может быть SHA-256, то вход в хеш должен быть определён детерминированно (через canonical JSON из Приложения A) и включать тип операции + payload + nonce (или другой источник уникальности)
