...
- **БД:** PostgreSQL (16+).
- **Кэш/очереди:** Redis (7+).
- **Асинхронность:** стандартные asyncio + библиотеки для работы с БД (asyncpg, aiopg).
- **Аутентификация:** JWT (с refresh‑токенами).
- **Подписи:** Ed25519 (или аналогичный алгоритм).
- **Документация:** OpenAPI/Swagger.
- **Логирование:** structured logging (JSON + Sentry).
- **Тестирование:** pytest + async‑тесты.

### 7.2. Frontend

- **Фреймворк:** Flutter (3.19+).
- **Стейт‑менеджмент:** Riverpod (или аналогичный).
- **API‑клиент:** Dio (или аналогичный).
- **Локализация:** стандартные инструменты Flutter.
- **Тестирование:**
  - юнит‑тесты: `flutter test`;
  - интеграционные тесты: `flutter drive`.
