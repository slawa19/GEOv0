...
`DB_POSTGRES_ISOLATION_LEVEL` — не выбор: с 2026-09-25 приложение принимает
только `SERIALIZABLE` (регистр и пробелы не важны, значение нормализуется) и
отказывается стартовать с любым другим уровнем (`RuntimeError` при построении
`Settings`, `app/config.py::_require_serializable_isolation`). Причина —
писатели лимита доверия (затухание, рост против параллельного повышения,
понижение линии через simulator trustline-update и `PATCH` кредитора) читают
долг без блокировки строк и корректны только потому, что `SERIALIZABLE`
обрывает цикл rw-зависимостей с параллельным платежом; ниже него лимит может
опуститься под закоммиченный долг (спека 019, Changelog 2026-09-25). Имя
оставлено ради совместимости с окружениями, где оно уже задано как
`SERIALIZABLE`.
