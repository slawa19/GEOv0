...
порты согласно своим параметрам и останавливают процесс только когда PID-файл
конкретного сервиса совпадает с PID listener-а на его настроенном порту. Чужой,
неподтверждённый или изменившийся listener не останавливается; `start`/`restart`
в таком случае завершается до запуска замены и сообщает конфликт владения.
