...
### Переход на миграцию `031` — удаление резервов

Миграция `031_drop_prepare_locks` (программа 019, стадия 5, 2026-09-25) сопровождает код без резервов:
платежи координируются `SERIALIZABLE`, повтором всей транзакции и одной advisory-идентичностью
эквивалента (разделяемой у платежей и инжекта, исключительной у клиринга; протокол §7.6.1). Смешанная
работа версий не поддерживается, поэтому переход — под той же остановкой всех денежных писателей (API,
клиринг, тики, инжект, admin). Миграция блокирует `prepare_locks`, **отказывает**, если в ней есть строки
(это значит, что работает писатель старой версии или пропущено осушение `030`), и только затем удаляет
таблицу; строки она сама не удаляет. История транзакций, долгов и журнала не трогается. Проверка до
применения: `SELECT count(*) FROM prepare_locks;` — `0`.

**Откат (`downgrade` ниже `031`) разрешён** под той же остановкой: восстанавливается полная пустая схема
таблицы (столбцы, умолчания, CHECK `lock_type`, первичный и уникальный ключи, оба внешних ключа с их
поведением удаления, все индексы, включая GIN по `effects`); строки не восстанавливаются.
