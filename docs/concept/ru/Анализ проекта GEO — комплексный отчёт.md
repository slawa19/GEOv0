...
| **Web-админка** | Jinja2 + HTMX/Alpine.js | Простота, без SPA |
...
| Web-админка (Jinja2 + HTMX) | Средний | 5–7 дней |
...
> **Примечание:** Код пока не написан, поэтому противоречий нет. Однако есть несколько мест, где документы расходятся между собой:
...
2. **Стек технологий**
   ...
   - **Рекомендация:** Финализировать выбор в README
