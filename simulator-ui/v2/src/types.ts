export type GraphSnapshot = {
  ...
  generated_at: string
  /**
   * Сколько раз к этому снапшоту применяли патчи (`F-013-2` / `T1303`, 2026-09-10).
   *
   * ЗАЧЕМ ОТДЕЛЬНОЕ ПОЛЕ, а не сдвиг `generated_at`. `generated_at` — отметка СЕРВЕРА о том,
   * когда собран снапшот; подделывать её на клиенте значит врать о происхождении данных, а
   * `useLayoutCoordinator.ts:279-283` отдельно предупреждает, что на неё нельзя вешать раскладку,
   * потому что реальные потоки двигают её и без изменения топологии.
   *
   * Патчи меняют ДАННЫЕ, не трогая отметку сервера, поэтому кэшам нужен признак «данные другие»,
   * а не «снапшот другой». Счётчик двигается в самом применителе патчей (`demo/patches.ts`), то
   * есть его нельзя пропустить, добавив нового вызывающего.
   */
