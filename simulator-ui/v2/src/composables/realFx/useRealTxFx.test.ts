...
type SparkEdge = { from: string; to: string }
...
describe('useRealTxFx helpers', () => {
  ...
  it('__pickSparkEdges: keeps short routes, collapses long routes to endpoints', () => {
    const shortEdges: SparkEdge[] = [{ from: 'A', to: 'B' }]
    expect(__pickSparkEdges(shortEdges, { from: 'A', to: 'B' })).toEqual(shortEdges)

    const longEdges: SparkEdge[] = [
      { from: 'A', to: 'B' },
      { from: 'B', to: 'C' },
      { from: 'C', to: 'D' },
    ]

    expect(__pickSparkEdges(longEdges, { from: 'A', to: 'D' })).toEqual([{ from: 'A', to: 'D' }])
    expect(__pickSparkEdges(longEdges, { from: '', to: 'D' })).toEqual([])
    expect(__pickSparkEdges(longEdges, undefined)).toEqual([{ from: 'A', to: 'D' }])
  })
})
