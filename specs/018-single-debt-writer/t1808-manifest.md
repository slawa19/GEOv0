...
| `tests/integration/test_p015_t1526_nan_amount_reaches_the_money_column_postgres.py` | REWRITE in place: no stand-down; `try` spans the fixture block (book flushes at exit); `test_c` raw INSERTs run inside a fixture operation (context set), NaN still `23514 chk_debt_amount_positive`. 4 tests |
...
| `tests/integration/test_p015_inject_retries_a_serialization_failure_postgres.py` | REWRITE in place: competitor Core `update(Debt)` inside its own fixture operation on its own session; `40001` still at the owner's flush (asserted). 1 test |
