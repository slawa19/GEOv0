...
- **Сессия `pay()` и фикстура mode-A** (`G2 C9`, `G3 C5`). HTTP-тесты mode A подставляют `db_session` через override `get_db` (`tests/conftest.py:596-597`). Если `pay()` открывает свою `SERIALIZABLE`-сессию или повторяет на свежей, он не видит засеянного в mode A, а его коммиты уходят мимо откатываемой внешней транзакции; ассерты-предпосылки `READ COMMITTED` в lost-update (`:184-191`), узком месте (`:122-128`) и skip (`:349-355`) начинают проверять сессию, которой платёж больше не пользуется, и остаются зелёными. Решить до `T1904`: первая попытка — на DI-сессии, либо HTTP-тесты платежа — mode B. **Решено 2026-09-25 (P2-7):** mode B с внедрённой фабрикой сессий для **каждой** попытки `pay()`; вариант «первая попытка на DI-сессии» отклонён — он разводит семантику первой попытки и повтора.
...
- **Поведение узкого места:** сохранённый `ABORTED` проигравшего (`:232-235`) после стадии 5 **зависит от допуска** (исправлено 2026-09-25, `FORK-5`; прежний безусловный вывод «становится „строки нет“» отклонён консультацией): проигравший, **допущенный** до отказа capacity, сохраняет окончательный `ABORTED` и повторяет его; не допущенный (отказ маршрутизации на свежем снимке до допуска) — строки нет. Исход назвать в `T1908` по фактическому порядку реализации; ни «чинить» записью `ABORTED` не допущенного, ни превращать допущенный отказ в повторно исполнимый запрос. Имена узлов lost-update и узкого места держит гард `test_p017_required_gate_runs_on_postgres.py:69-72` (`:418-422`) и документы `docs/en/10-testing-framework.md:99`, `docs/ru/runbook-dev-wsl2-docker-no-desktop.md:315`.
...
| :232-235 ⚑ | same | states {winner: COMMITTED, loser: ABORTED} | REWRITE IN PLACE (stage 5), **expectation depends on admission** (corrected 2026-09-25, third consultation `FORK-5`; the earlier unconditional "loser has no row" is rejected): a loser **admitted** before its capacity refusal keeps the definitive `ABORTED` and replays it, even when its fresh-snapshot retry meets the refusal before any insert; only a loser refused **before admission** (initial routing on a fresh snapshot) has no row. Name the actual outcome in T1908 against the implemented ordering; never turn an admitted refusal into a re-executable request. Stages 3-4 keep `ABORTED` (capacity refusal on recheck is definitive) |
...
#### `T1909` (стадия 5, часть b, 2026-09-25) — исполнение строк 5.1 и 5.3 о локах и резервах

- `DROP` исполнены (`7eb3b85`): `tests/integration/test_payment_pair_advisory_locks_postgres.py` (наследники — встречные направления `test_p019_t1908_lock_removal_experiments_postgres.py` (b), `40001` считается явно; `test_payment_inverse_multisegment_postgres.py`), `tests/integration/test_prepare_locks_tx_id_fk_postgres.py` (строка `:164-176` «TO WRITE» закрыта `tests/integration/test_p019_migration_031_postgres.py`: каталог `prepare_locks` после `031 → 030` равен каталогу цепочки `пусто → 030` — оба внешних ключа с `ON DELETE`, все индексы, включая GIN; `d540d04`), `tests/unit/test_clearing_prepare_lock_conflict.py` (исключения по резерву больше нет), `tests/unit/test_payment_engine_advisory_lock_key.py` (pair- и tx-ключей нет).
- `REWRITE IN PLACE` исполнены: `test_payment_engine_advisory_locks_execute.py` — пять узлов одной идентичности в двух режимах (разделяемый набор отсортирован и без повторов, исключительный сессионный и его освобождение — одна идентичность, staged восстанавливает `lock_timeout` вызывающего, связывание берёт разделяемый лок до первого чтения строки, один убывающий бюджет); `test_routing_reserved_and_policy.py` — тест 1 → `::test_build_graph_capacity_is_the_limit_and_respects_policy` (ёмкость 100, политика без изменений); `test_payment_prepare_capacity_policy.py` — три узла о резервах → ёмкость «лимит − долг + обратный долг» и собственные маршруты платежа; `test_payment_staged_multicall_postgres.py` → `::test_staged_shared_sets_do_not_wait_for_each_other_and_hold_off_a_clearing_exclusive_lock_postgres`; `test_clearing_payment_prepare_interlock_postgres.py` → `::test_clearing_exclusive_lock_blocks_a_reverse_payment_postgres`; `test_p015_inject_holds_the_owner_lock_postgres.py` — держатель теперь `ShareLock`; `test_clearing_skip_releases_locks_postgres.py`, `test_concurrent_*` — механизм ожидания по строке, ⚑ денег и порядка сохранены.
- **Новое (TO WRITE консультации, предусловие 6):** `tests/integration/test_p019_equivalent_lock_modes_postgres.py` — разделяемый/разделяемый, разделяемый/исключительный, свежий снимок после ожидания, очистка лока (4 исхода) и инвалидация при неподтверждённом освобождении; `tests/integration/test_p019_clearing_attempt_conflicts_reach_the_retry_owner_postgres.py` — конфликт в любом месте попытки клиринга. Все — обязательные узлы гейта (`tests/unit/test_p017_required_gate_runs_on_postgres.py`), вместе с тремя узлами повтора `23505` пары долга. Тест `FOR SHARE` по строке (`test_p019_clearing_holds_the_stop_for_share_postgres.py`) остаётся независимым от оставленного лока: выключатель снимает его, ассерт — на вызов выключателя.
...
#### `T1909` (стадия 5, часть b, 2026-09-25) — что стало с ⚑ 5.4

Форма локов `KEEP-EQUIVALENT-LOCK`: платёж, staged-фаза и инжект держат лок эквивалента разделяемым, клиринг — исключительным (сессионный, до снимка); admin и реакция сверки advisory-лока не берут. Строки «TO WRITE (stage 5)» выше закрыты так:

- `test_p015_step5c_hold_races_postgres.py`: `:215` (платёж ждёт hold) → **изменённый исход**, тест переименован `::test_step5c_p_a_reaction_arriving_between_prepare_and_commit_waits_for_the_payment` → `::test_step5c_p_a_reaction_arriving_between_the_binding_and_the_money_phase_holds_first_and_the_payment_is_refused`: hold коммитится первым, `FOR SHARE` платежа — `40001`, повтор `pay()` отказывает `equivalent_integrity_hold`, `ABORTED` (исход, который `T1544`/`T1546` допускают — spec §3 (В)); ⚑ отказ, долги без изменений, hold в силе сохранены. `:277-:293` (реакция ждёт `FOR SHARE` платежа) — ожидание по строке (`pg_locks`, не advisory), порядок payment → reaction и ⚑ сохранены. `:317-:338` → `::test_step5c_p_the_reaction_confirms_in_its_own_snapshot_taken_after_the_verdict`: лока реакции больше нет (`reconciliation.py` `react_to_failed`), ⚑ `HOLD_NOT_CONFIRMED` при починке до снимка реакции сохранён. `:371-:398` — клиринг встречает `40001` на строке и его владелец повторов отказывает по hold; `:398` (нет advisory-лока после клиринга) **сохранён** как проверка очистки исключительного сессионного лока. `:438-:448` — шов клиринга после чтения hold (вместо снятого `_locked_pairs_for_equivalent`); порядок clearing → reaction сохранён. `:538` → `::test_step5c_p_the_admin_clear_waits_for_a_holder_of_the_row`: снятие ждёт держателя строки, ⚑ `200` и снятый hold сохранены.
- `test_p015_b4_entries_and_money_postgres.py`, `test_p015_step5b_criterion_b_postgres.py`, `test_p015_t1525_control_postgres.py`: строки `_prepare_locks == 0` — `DROP` с таблицей; ⚑ денег, журнала, конверта, аудита без изменений.
...
**`T1909` (стадия 5, часть b, 2026-09-25) — что стало с ⚑ этого файла.** `PATCH` advisory-лока не берёт; граница — `FOR SHARE` строки эквивалента у каждого писателя денег до его коммита. Тест A (`:172`) переименован `::test_a_stop_arriving_between_prepare_and_commit_waits_for_the_payment` → `::test_a_stop_arriving_between_the_binding_and_the_money_phase_answers_first_and_the_payment_is_refused` — **изменённый исход**: `PATCH` отвечает первым, `FOR SHARE` денежной фазы — `40001`, повтор `pay()` отказывает стопом, `ABORTED` (второй из двух исходов `T1544`, spec §3 (В)); ⚑ отказ, долги без изменений, выключенный эквивалент сохранены. `:225`, `:304`, `:358`, `:462`, `:698`, `:784` — предпосылки ожидания advisory заменены ожиданием строки/транзакции по `pg_locks` и `pg_blocking_pids`, ⚑ порядков `["clearing","patch"]`, `["payment","patch"]` и отказов сохранены. `:246`, `:487`, `:562` (`prepare_locks`) — `DROP`, как записано; `:395` (`_no_advisory_lock_is_held`) **сохранён**, а не снят: после клиринга ни одного advisory-лока не остаётся — это теперь проверка очистки исключительного сессионного лока. Строки «(missing)» закрыты на стадии 2 `test_p019_owner_before_row_races_postgres.py`; на стадии 5 оба теста переименованы (суффикс `_waits_on_the_owner_lock` снят: ни один admin-путь advisory-лока не держит, ассертом), и расписание `[payment-hold_clear]` «деньги после admin» сменило исход: платёж встречает закоммиченный hold, повтор отказывает по hold, `ABORTED` (spec §3 (В)).
