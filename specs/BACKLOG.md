...
исчезают вместе с SQLite, то есть относятся к черновику **017**. Для `scripts/seed_db.py` соответствия нет. Якоря первой и третьей перепроверены оркестратором по коду; поведенческие утверждения
