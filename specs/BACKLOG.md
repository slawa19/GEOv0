...
- **Тест формы записи журнала может пройти на чужом CHECK.** `tests/unit/test_p015_b4a_journal_mechanism.py:1226` обещает краснеть при снятии `chk_debt_journal_entries_delta`, но вход `before=2, after=3, delta=0` отвергает и арифметический CHECK, а ассерт `:1274` принимает любой CHECK. Точное имя ограничения держат `tests/integration/test_p015_b4_entries_and_money_postgres.py:2426,2563`, так что эффект не потерян. P3.
