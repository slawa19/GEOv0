...
| `.snap` не покрыт `.gitattributes` — найдено 2026-08-21 | P3 | **Current:** `.gitattributes` перечисляет `*.ts`, `*.vue`, `*.json` и прочее как `eol=lf`, но `*.snap` не назван и попадает под `* text=auto`; vitest переписывает снапшот с CRLF, поэтому **обычный прогон `npm run test:unit` оставляет рабочее дерево грязным** без единого изменения содержимого. Наблюдено на `legacyWindowsMarkupSnapshots.test.ts.snap`: `git status` показывает файл изменённым, `git diff` пуст. **Intended:** зелёный прогон не меняет рабочее дерево. **Optimal:** одна строка `*.snap text eol=lf` — класс clean, продуктового выбора нет. Цена сегодня: ложный «грязный» статус перед коммитом и риск затащить EOL-шум в diff | `.gitattributes` (строки `*.ts`…`*.sh`); `simulator-ui/v2/src/legacyReference/__snapshots__/legacyWindowsMarkupSnapshots.test.ts.snap` |
