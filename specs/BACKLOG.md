...
| Пункт | Sev | Суть | Evidence |
|---|---|---|---|
| Trustline: наивные timestamps | P2 на SQLite / P3 на PG | `app/schemas/trustline.py` не имеет UTC-валидатора, который есть у соседних схем → на dev-пути SQLite время наивное, а UI сдвигает его повторно | `trustline.py:22-23`; потребители `admin-ui/src/pages/TrustlinesPage.vue:129`, `useGraphAnalytics.ts:32` |
| TODO-ESC: ESC не привязан к окну | P3 | Парная отложенная работа: обработка ESC в destructive-confirm глобальная, а не в границах окна-владельца. При стопке окон WM ESC во вложенном подтверждении может закрыть не ту поверхность. Контракт уже отслеживается тестом | `simulator-ui/v2/src/components/WindowShell.vue:45`; `useDestructiveConfirmation.ts:65`; тест `useDestructiveConfirmation.test.ts:45` |
...
| `tmp_*` скрипты под git | P3 | Отслеживаются без записи «оставлено намеренно»; плюс `Show-RecentLog` без вызывающих | `scripts/tmp_check_graph_isolates.js`, `scripts/tmp_sse_watch.py`, `scripts/fix_concatenated_admin_fixtures.py`, `scripts/verify_hybrid_approach.ps1`; `run_full_stack.ps1:378-402` |
...
| 5 | `app/api/v1/integrity.py:260-261` | `except Exception:` вокруг `compute_integrity_checkpoint_for_equivalent` внутри `POST /integrity/verify` (`:182`), до `await db.commit()` на `:287`. Запрос к БД — значит транзакция реально может быть отравлена | (а) |
