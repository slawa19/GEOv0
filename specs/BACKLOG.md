...
- **Остаток, класс 2 по §19.5 — внесено 2026-09-25, получатель — программа 020.** Импульс отмены вызывающего, пришедший, пока идёт разрешение, теряется, если это разрешение затем падает: `_drain_task` поднимает ошибку задачи (`app/core/clearing/service.py:202`) до того, как вернуть запомненную отмену, а блок разрешения коммита (`:2206`, `:2229`) тогда считает её отсутствующей. Существовало до 020; денег не двигает (исход коммита решает та же классификация), теряется только признак отмены. Не воспроизведено.
