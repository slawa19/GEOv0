...
В коде `TrustLineService` пишет ровно два значения — `'active'` при создании (`app/core/trustlines/service.py:221`) и `'closed'` при закрытии (`:494`). Записи `'frozen'` нет нигде; поиск по `app/` находит только фильтры на чтение. Роутер при этом отбирает исключительно `status == 'active'`, то есть даже возникни такая линия, маршруту она не достанется — что расходится с `:1833-1834`.
