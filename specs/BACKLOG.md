...
- **Закрыто 2026-09-25, программа 020 стадия 1** (`d448284`, ветка `claude/020-stage1`). Воспроизведено до правки (инъекция сбоев, расписание по P2-3 второго круга Codex): неизвестный коммит (инъекция), первое полное разрешение падает настоящим `40P01`, второе (классификатора) честно не находит вхождения — на базе 2 попытки и `30` очищенных. Правка: провал разрешения больше не классифицируется сам — блок разрешения коммита поднимает ошибку (или отмену) самого COMMIT, как при «вхождение не найдено». Тест `tests/integration/test_p020_unknown_commit_is_not_retried_when_the_resolver_fails_postgres.py::test_an_unknown_commit_whose_resolver_deadlocks_is_not_retried` с контролями: подтверждённый PostgreSQL `40001` на COMMIT повторяется, закоммиченное вхождение за потерянным подтверждением возвращает свою сумму, неразрешённый неизвестный коммит — `E010` без повтора. Сознательное следствие: закоммиченное вхождение, чей резолвер упал, теперь тоже `E010` (раньше его находило второе разрешение); повтор того же цикла находит его по детерминированному `tx_id`.
