...
| Непроверенные касты в API-клиентах — simulator `[x]` 2026-08-12, Admin `[!]` | P3 | Simulator: девять оставшихся action/list/target 2xx shapes теперь проходят `simulatorContractJson`; обязательные ids/arrays/integers, canonical aliases и bounded plain-decimal strings проверяются до composables (`simulatorApi.ts:197-362`, decoders `simulatorContracts.ts:637-681`). Admin paginated endpoints без Zod остаются отдельным открытым срезом | Simulator RED до decoder — exit `1`, `9 failed / 23 passed`, каждое malformed 2xx было принято; green contract + downstream — exit `0`, `44 passed`. Первый typecheck после временного сужения общего snapshot-типа — exit `1`, две ошибки по optional `reverse_used`; backend decoder оставлен strict, общий fallback type возвращён к optional, повторный typecheck — exit `0`. Build (`DEBUG=false`) и lint — exit `0`; diff-check — exit `0`. Open: Admin `realApi.ts:663,751,778,796,871` |
...
| Мёртвые экспорты — частично закрыто 2026-08-12 | P3 | `[x]` Неиспользуемые `PaymentRouter.find_paths`/Yen и UI `restartRun` удалены после нулевого runtime-reference scan; policy/max-hop проверки перенесены на живой `find_flow_routes` (`router.py:460-550`, `test_routing_reserved_and_policy.py:177-256`). `[!]` `bestEffortTotal` пока остаётся активным: удалить только вместе с runtime-валидацией обязательных pagination fields, а не раньше | До: `router.py:552-630`, `simulatorApi.ts:127-131`; после: `heapq`, `find_paths`, `restartRun` и Yen-only test отсутствуют, active-tree reference scan — `0`. Первый canonical backend запуск не дошёл до pytest: exit `2`, ошибочный nonexistent selector `test_routing_determinism.py`; исправленный selector — exit `0`, `7 passed`. Simulator contract — exit `0`, `14 passed`; typecheck/build (`DEBUG=false`) — exit `0`; pinned Ruff `0.1.14` и diff-check — exit `0`. Открытый owner: `realApi.ts:540-548` |
...
На 2026-08-11 закрыты независимые M20-срезы 1–3, 4–5 и 6–11; пункты 12–13 остаются открытыми и
не входят в evidence этих коммитов.
