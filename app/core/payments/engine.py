...
class PaymentEngine:
    def __init__(self, session: AsyncSession):
        ...
        self._retry_base_delay_s = getattr(
            settings, "DB_RETRY_BASE_DELAY_SECONDS", 0.05
        )
    ...
    async def _commit_with_retry(self) -> None:
        ...
        while True:
            try:
                ...
                attempt += 1
                if attempt >= self._retry_attempts or not self._is_retryable_db_error(
                    exc
                ):
                    raise
    ...
    async def prepare_routes(
        ...
        *,
        ...
        """
        logger.info(
            "event=payment.prepare_multipath tx_id=%s routes=%s", tx_id, len(routes)
        )
        try:
            PAYMENT_EVENTS_TOTAL.labels(event="prepare", result="start").inc()
        except Exception:
    ...
    async def commit(self, tx_id: str, *, commit: bool = True):
        ...
        if commit:
            ...
            # NOTE: deadlock-retry is implemented only for commit() via `_commit_with_retry()`.
            # `flush()` is not retried; when called under `begin_nested()` (e.g. simulator
            # real-mode tick), this is acceptable.
            await self.session.flush()
