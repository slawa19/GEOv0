...
from sqlalchemy import CheckConstraint, DateTime, ForeignKey, Numeric, UniqueConstraint, Uuid, func
...
class Debt(Base):
    ...
    debtor_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), ForeignKey('participants.id'), nullable=False, index=True)
    creditor_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), ForeignKey('participants.id'), nullable=False, index=True)
