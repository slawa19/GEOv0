...
Текущая программа payment-integrity review: `specs/002-payment-integrity-follow-up/spec.md`;
последовательность фаз — `plan.md`, исполнимый backlog — `tasks.md` в том же
каталоге. Программа `specs/001-codebase-renovation/` завершена и остаётся
историческим evidence, а не активным backlog. Статусы документов не заменяют
проверку кода, git и свежих gate-результатов.
...
Его раздел 0 фиксирует постоянную авторизацию владельца на автономное выполнение
явно начатой фазы без повторных запросов для обычных repo/commit/push/CI/E2E и
строго read-only review действий внутри утверждённого scope.
