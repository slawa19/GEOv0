...
**Функциональные требования:**
1. Долги: исключить одновременное существование A→B и B→A в одном эквиваленте как независимых записей **только если** модель предполагает единственную направленность.
   - В текущей реализации v0.1 долги хранятся как направленные записи `Debt(debtor, creditor)` и **допускают** существование обеих сторон одновременно; консистентность обеспечивается бизнес-логикой (погашение встречного долга в `commit()`).
   - Следовательно, этот пункт закрывается как **SPEC уточнение**, а не как обязательный DB CHECK.
...
**Статус реализации (v0.1, 2026-01-04):**
- (1) Частично: добавлен `X-Request-ID` middleware и прокидывание `request_id` в `log_duration` (best-effort корреляция); события payment/clearing пишутся структурировано на уровне сервисов/engine.
- (2) Реализовано: добавлен `/metrics` (Prometheus) + middleware для `geo_http_requests_total` и `geo_http_request_duration_seconds`; добавлены бизнес-счётчики `geo_payment_events_total`, `geo_clearing_events_total`, `geo_routing_failures_total`.
- (4) Реализовано: добавлена recovery-петля (startup + periodic) для очистки истёкших `prepare_locks` и аборта зависших PAYMENT транзакций.
- Integrity checkpoints (DB-M10): добавлено периодическое вычисление и запись `integrity_checkpoints` по всем эквивалентам.
...
**Статус реализации (v0.1, 2026-01-04):**
- Добавлен интеграционный тест multipath split на 2 маршрута (routing + prepare_routes + commit).
- Добавлены контрактные тесты: проверка соответствия `api/openapi.yaml` и фактических путей/методов FastAPI (drift detection).
