"""Equivalent writers refuse a non-canonical row; a legacy precision is read and repaired.

WHERE A LEGACY ROW CAN EXIST (programme 017). A row whose CODE is outside `^[A-Z0-9_]{1,16}$` can
only exist in a database without `chk_equivalents_code_format`. Migration 007 installs that CHECK on
PostgreSQL, validated against the rows already there, and every writer in the application refuses
such a code (`app/utils/validation.py`) - so on the only backend the application runs on the row
cannot be written at all. The two tests that seeded a legacy CODE did so on a SQLite file of their
own, because only there was the premise possible; they left with SQLite (017 stage 3, slice S3). A
legacy PRECISION is reachable on PostgreSQL - no CHECK bounds it - and its tests stay here.
"""
