...
SINCE 018 B0b the stand runs on a disposable clone of the migrated template (`committed_database`),
and the clone's drop is the whole cleanup. The per-id `_cleanup` deleted journal rows, which stage B1
refuses; the check after each test read the shared tier, where a clone test leaves nothing, so it
would pass vacuously - both went.
"""
