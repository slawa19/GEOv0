...
@pytest.mark.asyncio
@pytest.mark.parametrize("fail_on_call", [1, 2])
async def test_close_checkpoint_failure_is_not_swallowed_or_committed(
    db_session,
    monkeypatch,
    fail_on_call: int,
) -> None:
    ...
    sender = Participant(
        id=uuid.uuid4(),
        pid="close-audit-owner",
        display_name="Close audit owner",
        public_key="test-public-key",
        type="person",
        status="active",
        profile={},
    ...
    receiver = Participant(
        id=uuid.uuid4(),
        pid="close-audit-peer",
        display_name="Close audit peer",
        public_key="test-peer-key",
        type="person",
        status="active",
        profile={},
