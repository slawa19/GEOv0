...
now SEES the lock: it is a test of the measurer (AGENTS.md §15), not of the concurrency the lock
...
async def _held_owner_locks(session) -> set[tuple[int, int]]:
    ...
    return {(int(row.classid), int(row.objid)) for row in rows}
...
async def test_the_owner_lock_is_executed_and_held_on_the_default_tier(db_session) -> None:
    ...
    `_acquire_equivalent_owner_locks` (the shape of the pre-017 `if not self._is_postgres()` guard),
    and the set below stays empty.
    """
    ...
    assert expected <= held, (
        f"the default tier did not take the equivalent owner lock: expected {sorted(expected)} "
